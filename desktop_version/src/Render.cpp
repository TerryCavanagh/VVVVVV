...
static void rendermaplegend(void)
{
    ...
    for (size_t i = 0; i < map.teleporters.size(); i++)
    {
        ...
        if (x >= 0 && y >= 0 && x < data.width && y < data.height)
        {
            ...
            {
                graphics.drawtile(data.legendxoff + (x * 12 * data.zoom), data.legendyoff + (y * 9 * data.zoom), 1127 + tile_offset);
            }
            else if (map.showtargets && !map.isexplored(x + data.startx, y + data.starty))
            {
                graphics.drawtile(data.legendxoff + (x * 12 * data.zoom), data.legendyoff + (y * 9 * data.zoom), 1126 + tile_offset);
            }
        }
    }
    ...
    if (map.showtrinkets)
    {
        for (size_t i = 0; i < map.shinytrinkets.size(); i++)
        {
            if (!obj.collect[i])
            {
                ...
                if (x >= 0 && y >= 0 && x < data.width && y < data.height)
                {
                    graphics.drawtile(data.legendxoff + (x * 12 * data.zoom), data.legendyoff + (y * 9 * data.zoom), 1086 + tile_offset);
                }
            }
        }
    }
